...
assert.equal(rolling.length, 3, "the final pre-checkpoint runway must contain August 14 through 16 only");
assert.equal(rolling[0].date, "2026-08-14", "the active rolling window must start today in America/Denver");
...
assert.deepEqual(SCHEDULE.find(item => item.date === "2026-08-14").tasks, ["move_stage_aug14"], "today must retain the current staging-area reset");
assert.equal(SCHEDULE.some(item => item.date < "2026-08-14"), false, "expired pre-August-14 rows must not remain active");
...
assert.equal(oneTaskForDate("2026-08-14"), "move_stage_aug14", "today's rolling task must appear");
pushed.move_stage_aug14 = "2026-08-15";
assert.equal(oneTaskForDate("2026-08-14"), null, "a pushed-away task must not be replaced by unrelated housework");

...
assert.equal(rolling.length, 2, "the final pre-checkpoint runway must contain August 15 through 16 only");
assert.equal(rolling[0].date, "2026-08-15", "the active rolling window must start today in America/Denver");
...
assert.deepEqual(SCHEDULE.find(item => item.date === "2026-08-15").tasks, ["move_decor_aug15"], "today must retain the current decor box");
assert.equal(SCHEDULE.some(item => item.date < "2026-08-15"), false, "expired pre-August-15 rows must not remain active");
...
assert.equal(oneTaskForDate("2026-08-15"), "move_decor_aug15", "today's rolling task must appear");
pushed.move_decor_aug15 = "2026-08-16";
assert.equal(oneTaskForDate("2026-08-15"), null, "a pushed-away task must not be replaced by unrelated housework");
